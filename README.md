# 🇦🇷 Radios Argentinas para Euro Truck Simulator 2

**500 radios argentinas funcionales** listas para usar en el radio browser de ETS2/ATS.

## 📥 Instalación

1. Descargá `live_streams.sii`
2. Copialo a la carpeta de tu perfil:

```
Documentos/Euro Truck Simulator 2/profiles/<TU-PERFIL>/
```

3. Reiniciá el juego
4. Abrí el radio browser (tecla `R` por defecto) → vas a ver **"Radios Argentinas"** en la lista de estaciones

> También funciona en American Truck Simulator (misma ruta con `American Truck Simulator`).

## 🎵 Qué incluye

- **500 estaciones** de Argentina, todas probadas y funcionando al momento de generar el archivo
- Géneros: Rock, Pop, Tango, Cumbia, Folklore, Noticias, Electrónica, Clásica y más
- Formato estándar: `URL|Nombre|Género|AR|Bitrate|es`
- Bitrates de 32 a 320 kbps

## 🔄 Regenerar la lista

Este repo contiene solo el archivo generado. El scraper que lo produce (con la API de radio-browser) vive aparte; las estaciones caídas se pueden re-testear y regenerar en cualquier momento.

## 📅 Última actualización

- **Fecha:** 2026-08-07
- **Estaciones probadas:** 859 → 500 funcionales incluidas
